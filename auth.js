...
function notifyBackendLogout(reason = "Logged out.") {
    ...
    try {
        fetch(
            "https://__PUBLIC_WORKER_URL__/register-session",
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: sessionPayload,
...
async function searchAI() {
  ...
  try {
    ...
    const res =
      await fetch(
        "https://__PUBLIC_WORKER_URL__/ai-search",
        {
          method: "POST",
...
async function submitTOTP() {
  ...
  try {
    const BACKEND = window.BACKEND_URL || "https://__PUBLIC_WORKER_URL__"; // ← update this
...
async function runAIIndexingOnLogin() {
    ...
    try {
        const checkRes = await fetch('https://__PUBLIC_WORKER_URL__/ai-index-status', {
            method: 'POST',
            headers: {
    ...
    if (!allFiles.length) {
        ...
        try {
            const res = await fetch("https://__PUBLIC_WORKER_URL__/files.json", {
    ...
    try {
        const progressRes = await fetch('https://__PUBLIC_WORKER_URL__/ai-chunk-status-all', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
    ...
    for (const fileEntry of pdfFiles) {
        ...
        try {
            ...
            const dlRes = await fetch(
                `https://__PUBLIC_WORKER_URL__/docs/${encodeURIComponent(filePath)}`,
                { headers: { "Authorization": `Bearer ${token}` } }
            );
            if (!dlRes.ok) {
            ...
            for (let i = 0; i < chunks.length; i++) {
                try {
                    const chunkRes = await fetch("https://__PUBLIC_WORKER_URL__/ai-index", {
                        method: "POST",
                        headers: { 
                            "Content-Type": "application/json", 
                            "Authorization": `Bearer ${token}` 
            ...
            await fetch('https://__PUBLIC_WORKER_URL__/ai-file-indexed', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    try {
        await fetch('https://__PUBLIC_WORKER_URL__/ai-index-status', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
async function deleteFileChunks(fileName) {
  const token = sessionStorage.getItem('vaultSessionToken') ||
                sessionStorage.getItem('vaultSession') || '';
  try {
    await fetch('https://__PUBLIC_WORKER_URL__/ai-chunk-delete', {
      method: 'POST',
...
async function indexAI(fileUrl, fileName) {
  ...
  try {
    const checkRes = await fetch(
      'https://__PUBLIC_WORKER_URL__/ai-chunk-status',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  if (!fullText || fullText.trim().length < 20) {
    ...
    await fetch('https://__PUBLIC_WORKER_URL__/ai-index', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  for (let i = 0; i < chunks.length; i++) {
    try {
      const res = await fetch(
        'https://__PUBLIC_WORKER_URL__/ai-index',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
...
async function sendAIMessage() {
  ...
  try {
    ...
    if (!token.startsWith('offline-')) {
      try {
        let res = await fetch('https://__PUBLIC_WORKER_URL__/ai-search', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        if (res.status === 401 && typeof _silentReAuth === 'function') {
          ...
          if (fresh && !fresh.startsWith('offline-')) {
            ...
            sessionStorage.setItem('vaultSessionToken', fresh);
            sessionStorage.setItem('vaultSession', fresh);
            res = await fetch('https://__PUBLIC_WORKER_URL__/ai-search', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
function onCaptchaSuccess(){
   ...
   async function sendLoginEmail() {
    try {
        ...
        await fetch('https://__PUBLIC_WORKER_URL__/login-email', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                email, name, purpose,
   ...
   async function sendSecurityAlert(reason){
    ...
    try{
        ...
        await fetch(
        "https://__PUBLIC_WORKER_URL__/security-alert",
        {
...
let _forceLogoutInterval = null;
    ...
    _forceLogoutInterval = setInterval(async ()=>{
        ...
        try{
            ...
            const res = await fetch(
                "https://__PUBLIC_WORKER_URL__/check-session",
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
