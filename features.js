...
// ============ GLOBAL CONFIG ============
window.BACKEND_URL = 'https://__PUBLIC_WORKER_URL__'; // single source of truth for backend URL
...
async function generateShareLink(){
  ...
  if(!masterPwd) {
    ...
    try{
        ...
        const res =
        await fetch(
            'https://__PUBLIC_WORKER_URL__/create-share',
            {
                method:'POST',
                headers:{
                    'Content-Type':
...
async function checkDocExpiryReminders(){
    ...
    try{
        await fetch(
            'https://__PUBLIC_WORKER_URL__/expiry-reminder',
            {
                method:'POST',
...
async function downloadSelectedAsZip() {
  ...
  try {
    ...
    var passRes = await fetch('https://__PUBLIC_WORKER_URL__/get-secret', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (!buffer) {
        ...
        var res = await fetch('https://__PUBLIC_WORKER_URL__/' + fetchPath, {
