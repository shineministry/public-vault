...
const res = await fetch(
    "https://__PUBLIC_WORKER_URL__/get-logs",
    {
        method: "POST",
...
async function requestPasskeyAccess() {
    ...
    try {
        const initRes = await fetch("https://__PUBLIC_WORKER_URL__/request-passkey", {
            method: "POST",
    ...
    const checkInterval = setInterval(async () => {
        try {
            const pollRes = await fetch("https://__PUBLIC_WORKER_URL__/check-passkey", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
